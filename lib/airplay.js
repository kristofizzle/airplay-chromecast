...
AirPlay.prototype.announce = function (service) {
  return new Promise(function (resolve, reject) {
    // Remove trailing '(X)' in names of Chromecast Audio groups.
    var strippedName = service.name.replace(/\s*\(\d+\)$/, "")
    this.server = new AirTunesServer({
      serverName: strippedName
    })
    ...
    this.server.on('volumeChange', function (volume) {
      ...
      var newVol = 1 - Math.abs(Math.max(volume, -30) / 30)
      return that.chromecast.setVolume({level: newVol})
